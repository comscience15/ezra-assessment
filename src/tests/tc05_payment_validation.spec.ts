...
test('TC05: Payment validation blocks Continue with incomplete card details @tag: p0_test_cases @tag: payment_validation', async ({ page }) => {
    ...
    await pay.fillCard(stripeCards.decline, true);
    await pay.continue();

    await pay.expectCardDeclinedError(true);
});
